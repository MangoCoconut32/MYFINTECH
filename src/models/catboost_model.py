"""CatBoostModel implementation for the MYFINTECH pipeline."""
...
class CatBoostModel(BaseFinanceModel):
    """Calibrated CatBoost classifier with internal 5-fold CV tuning."""
    ...
    def fit(
        ...
        logger.info("CatBoostModel: fitting on %d samples.", len(X_train))
        base_cat = CatBoostClassifier(**{**self.params, **self._fixed_params()})
    ...
    def save(self, path: str) -> None:
        ...
        with open(path, "wb") as fh:
            pickle.dump(self, fh)
        logger.info("CatBoostModel saved to %s", path)

    @staticmethod
    def _fixed_params() -> dict[str, Any]:
        return {"verbose": 0, "allow_writing_files": False}
