...
class PipelineOrchestrator:
    ...
    def _run_audit(self) -> None:
        ...
        dice_data = X_tv.copy()
        dice_data[self.cfg.xai.outcome_name] = y_tv.values

        dice_explainer = DiCEExplainer(
            model=model,
            train_df=dice_data,
            continuous_features=list(self.cfg.xai.continuous_features),
            outcome_name=self.cfg.xai.outcome_name
        )
        ...
        if not rejected.empty:
            cf = dice_explainer.generate_counterfactuals(
                rejected,
                total_CFs=self.cfg.xai.n_counterfactuals,
                features_to_vary=list(self.cfg.xai.features_to_vary)
            )
