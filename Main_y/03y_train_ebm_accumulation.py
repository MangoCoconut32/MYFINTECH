"""
=============================================================================
03y_train_ebm_accumulation.py — GLASSBOX CHAMPION FOR ACCUMULATION
=============================================================================
PURPOSE:
    Trains an Explainable Boosting Machine (EBM / GA2M) exclusively for
    AccumulationInvestment using the TREE VIEW of Master Dataset Y (30 raw features).
...
BENCHMARK TARGET:
    XGBoost Giga-Baseline AUC = 0.8846 (02y)
...
OUTPUTS (Output/Pipeline_Y/):
    03y_ebm_acc_model.pkl                  — Serialized EBM (pickle)
    03y_ebm_acc_global_explanation.html    — Interactive Shape Function dashboard
    03y_ebm_acc_feature_importance.png     — Feature importance bar chart
    03y_ebm_acc_results.json              — AUC, Brier, P, R, F1 vs XGB reference
=============================================================================
"""
...
print("=" * 70)
print("03y_train_ebm_accumulation.py — EBM Glassbox (Accumulation)")
print("=" * 70)
...
# Model
pkl_path = os.path.join(OUT_DIR, "03y_ebm_acc_model.pkl")
...
json_path = os.path.join(OUT_DIR, "03y_ebm_acc_results.json")
...
# Use ' & ' instead of ' x ' for interaction detection (Bug #10)
...
png_path = os.path.join(OUT_DIR, "03y_ebm_acc_feature_importance.png")
fig.savefig(png_path, dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"      Saved chart : {os.path.basename(png_path)}")

# --- 6b. Interactive HTML dashboard (global explanation) ---
html_path = os.path.join(OUT_DIR, "03y_ebm_acc_global_explanation.html")
try:
...
fig.tight_layout()
shape_path = os.path.join(OUT_DIR, "03y_ebm_shape_functions.png")
fig.savefig(shape_path, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"      Saved shapes: 03y_ebm_shape_functions.png")

# --- 6d. Performance Curves (ROC & Calibration) ---
print(f"      Generating Performance Comparison Curves (EBM vs XGB reference)...")
try:
    with open(os.path.join(OUT_DIR, "02y_xgb_acc_calibrated.pkl"), "rb") as f:
    ...
    perf_path = os.path.join(OUT_DIR, "03y_ebm_acc_performance_curves.png")
...
# ---------------------------------------------------------------------------
print("\n" + "=" * 70)
print("✅ 03y_train_ebm_accumulation.py COMPLETE")
print("=" * 70)
print(f"\n  Target       : {TARGET}")
...
print(f"\n  Outputs → {OUT_DIR}")
for fname in [
    "03y_ebm_acc_model.pkl",
    "03y_ebm_acc_results.json",
    "03y_ebm_acc_feature_importance.png",
    "03y_ebm_acc_global_explanation.html",
]:
