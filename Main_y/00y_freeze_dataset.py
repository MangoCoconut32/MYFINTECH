"""
=============================================================================
00y_freeze_dataset.py — DATASET FREEZER FOR PIPELINE X
=============================================================================
PURPOSE:
    This script creates Dataset_Needs_SOTA.csv — the immutable "Bible" of
    Pipeline Y. It encodes the cross-validation fold assignments directly
...
OUTPUT_CSV = os.path.join(_PROJECT_ROOT, "Dataset_Needs_SOTA.csv")

TARGET_STRATIFY = "IncomeInvestment"   # Most imbalanced → tightest stratification
...
# ---------------------------------------------------------------------------
print("=" * 68)
print("00y_freeze_dataset.py — Pipeline Y Dataset Freezer")
print("=" * 68)
...
print(f"      Phase 1: Stratifying Test set (1000 rows)...")
...
print("-" * 68)
print(f"{'Fold':>6}  {'N rows':>7}  {'Income=1':>10}  {'Income rate':>12}")
print("-" * 68)
...
for fold_id in sorted(df["stratified_fold"].unique()):
    ...
    block = df[mask]
    n     = len(block)
    pos   = block[TARGET_STRATIFY].sum()
    rate  = pos / n if n > 0 else 0.0
    label = str(fold_id) if fold_id >= 0 else "Test (-1)"
    print(f"{label:>6}  {n:>7}  {int(pos):>10}  {rate:>11.2%}")
...
fold0_rate = df[df["stratified_fold"] == 0][TARGET_STRATIFY].mean()
test_rate  = df[df["stratified_fold"] == -1][TARGET_STRATIFY].mean()
...
print("\n" + "=" * 68)
print("✅ Dataset frozen. Dataset_Needs_SOTA.csv is the Pipeline Y Bible.")
