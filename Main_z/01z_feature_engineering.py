"""
=============================================================================
01z_feature_engineering.py — MASTER DATASET BUILDER FOR PIPELINE Z
=============================================================================
PURPOSE:
    Fuses two feature-engineering "souls" into a single Master Dataset Z:
...
DUAL-DATA ARCHITECTURE:
    ...
    TREE VIEW (Train/Test_Master_Z_Tree.csv):
        Raw, unscaled values (e.g. Age=45, Wealth=250000).
        Used by: 02z (XGBoost Baseline), 03z (EBM Accumulation).
        Benefit: EBM Shape Functions are human-readable (e.g. "Wealth > 200000").
        MIFID rules in 05z use raw values (e.g. if Age > 65, if RiskPropensity < 0.4).

    NN VIEW (Train/Test_Master_Z_NN.csv):
        All continuous features scaled to [0, 1] via MinMaxScaler.
        Used by: 04z (TabNet SSL+MTL).
...
OUTPUTS (in BuisnessCase2/Output/Pipeline_Z/):
    Train/Test_Master_Z_Tree.csv — 30 features, RAW values (for EBM, XGBoost)
    Train/Test_Master_Z_NN.csv  — 30 features, [0,1] scaled (for TabNet)
    Train/Test_Master_Z.csv     — backwards-compat alias → Tree view
...
# ---------------------------------------------------------------------------
# Import Pipeline Z data contract
# ---------------------------------------------------------------------------
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _SCRIPT_DIR)
from utilsz import get_full_train_val, get_test_set, TARGET_COLS, PipelineXTransformer
...
OUT_DIR      = os.path.join(_PROJECT_ROOT, "Output", "Pipeline_Z")
...
print("=" * 70)
print("01z_feature_engineering.py — Master Dataset Z Builder")
print("=" * 70)

# ===========================================================================
# STEP 1 — Load raw data (utilsz contract - BASE STAGE)
# ===========================================================================
print("\n[1/7] Loading raw data via utilsz (BASE STAGE)...")
...
# ===========================================================================
print("\n[6/7] Assembling Symmetric Master Dataset Z (Dual-Data)...")
...
# --- Save Master View (Unified Bible for Pipeline Z) ---
# This file contains ALL 5000 rows, all 15 features, and the fold assignments.
master_path = os.path.join(OUT_DIR, "Master_Needs_SOTA_Z.csv")
...
train_tree.to_csv(os.path.join(OUT_DIR, "Train_Master_Z_Tree.csv"), index=False)
test_tree.to_csv(os.path.join(OUT_DIR, "Test_Master_Z_Tree.csv"), index=False)
shutil.copy(os.path.join(OUT_DIR, "Train_Master_Z_Tree.csv"), os.path.join(OUT_DIR, "Train_Master_Z.csv"))
shutil.copy(os.path.join(OUT_DIR, "Test_Master_Z_Tree.csv"), os.path.join(OUT_DIR, "Test_Master_Z.csv"))
...
train_nn.to_csv(os.path.join(OUT_DIR, "Train_Master_Z_NN.csv"), index=False)
test_nn.to_csv(os.path.join(OUT_DIR, "Test_Master_Z_NN.csv"), index=False)
...
legend_lines = [
    "=" * 70,
    "PIPELINE Z — SYMMETRIC FEATURE LEGEND (MIFID II)",
    "=" * 70,
    "",
...
print("\n" + "=" * 70)
print("✅ 01z_feature_engineering.py COMPLETE (SYMMETRIC HARDENING)")
print("=" * 70)
