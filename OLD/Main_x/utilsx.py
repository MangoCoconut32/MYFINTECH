"""
=============================================================================
utilsx.py — DOUBLE-STAGE DATA CONTRACT FOR PIPELINE X
=============================================================================
PURPOSE:
    The single, authoritative interface for all data access in Pipeline X.
    Supports two stages with strict caching:
      1. STAGE="base"   : Reads from Dataset_Needs_SOTA.csv (7 raw features)
      2. STAGE="master" : Reads from Master_Needs_SOTA_X.csv (15 engineered)
=============================================================================
"""
...
BASE_DATA_PATH   = os.path.join(PIPELINE_X_DIR, "Dataset_Needs_SOTA.csv")
MASTER_DATA_PATH = os.path.join(PIPELINE_X_DIR, "Master_Needs_SOTA_X.csv")
...
def _get_df(stage="master") -> pd.DataFrame:
    ...
    if stage not in ["base", "master"]:
        raise ValueError(f"[utilsx] Invalid stage '{stage}'. Use 'base' or 'master'.")
    ...
    if not os.path.exists(path_to_load):
        if stage == "master":
            raise FileNotFoundError(f"[utilsx] Master dataset not found at {path_to_load}. Run 01x first.")
        else:
            raise FileNotFoundError(f"[utilsx] Base dataset not found at {path_to_load}. Run 00x first.")
    ...
    # (Since 02x and 03x rely on the master features)
    if stage == "master":
        FEATURE_COLS = [
...
def get_train_fold(fold_id: int, stage="master") -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if fold_id not in range(5):
        raise ValueError(f"[utilsx] fold_id must be 0-4, got {fold_id}")
...
def get_raw_professor_data() -> pd.DataFrame:
    ...
    if not os.path.exists(RAW_PROFESSOR_PATH):
        raise FileNotFoundError(f"[utilsx] Raw professor file not found at {RAW_PROFESSOR_PATH}")
...
if __name__ == "__main__":
    print("=" * 68)
    print("utilsx.py — Double-Stage Contract Test")
    print("=" * 68)
    try:
    ...
    try:
        ...
        print(f"ℹ️  Stage 'master' Note: {e} (Expected if 01x hasn't run)")
